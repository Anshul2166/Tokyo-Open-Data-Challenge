...
let loginValidations = [check('email').isEmail(), check('password').isLength({ min: 5 })];
...
let changePasswordValidation = [check('password').isLength({ min: 5 }), check('oldPassword').isLength({ min: 5 })];
	...
	try {
		...
		if (user.validPassword(password)) {
			...
			req.logIn(user, err => {
				...
				return res.status(200).send({ message: 'The user is successfully logged in ' });
			});
		} else {
...
//@access Public
router.get('/auth/facebook/callback', passport.authenticate('facebook', { session: true }), (req, res) => {
