...
export const localSignup = (credentials,redirectOnSuccess) => async dispatch => {
  try {
...
export const localLogin = (credentials,redirectOnSuccess) => async dispatch => {
  try {
    console.log("User action=locallogin");
    const response = await axios.post("http://localhost:8080/api/users/login",credentials,{withCredentials:true});
    console.log("Done with response");
    console.log(response);
    if (response.status === 200) {
      console.log("Here is the 200 status");
